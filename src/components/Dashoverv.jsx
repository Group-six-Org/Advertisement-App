...
return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">Here's a summary of your advertisements.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <ul>
          {[
            { title: "Luxury Car for Sale", status: "Active", date: "March 20, 2025" },
            { title: "Apartment for Rent", status: "Pending", date: "March 18, 2025" },
          ].map((ad, index) => (
            <li key={index} className="border-b py-3 flex justify-between">
              <span className="font-medium">{ad.title}</span>
              <span className={`px-3 py-1 rounded-full text-sm ${
                ad.status === "Active" ? "bg-green-200 text-green-700" : "bg-yellow-200 text-yellow-700"
              }`}>
                {ad.status}
              </span>
